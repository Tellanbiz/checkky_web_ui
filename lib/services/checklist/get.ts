...
export async function getChecklists(name?: string, status?: string): Promise<CheckList[]> {
    var url = `/checklists?name=${name ?? "none"}&status=${status ?? "none"}`
    const res = await clientV1.get<CheckList[]>(url, {
        headers: {
